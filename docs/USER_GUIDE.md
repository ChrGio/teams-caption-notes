...
For a quick start and the Windows download, see the [main page](../README.md).
For source installation, testing, and packaging, see [Development](DEVELOPMENT.md).

- [Tray controls](#standalone-system-tray-app)
- [First-time caption setup](#first-time-caption-setup)
- [Updating](#updating-the-portable-app)
- [Troubleshooting](#if-no-captions-are-found)
- [Privacy and limitations](#privacy-and-operational-notes)
...
```powershell
.\.venv\Scripts\python.exe .\src\teams_caption_notes.py --output .\transcripts\weekly-sync.md --leave-grace 12
```

To save one meeting and exit automatically after you leave:

```powershell
.\.venv\Scripts\python.exe .\src\teams_caption_notes.py --exit-after-meeting
```
...
```powershell
.\.venv\Scripts\python.exe .\src\teams_caption_notes.py --diagnose .\teams-uia-diagnostic.json
```
...
```powershell
.\.venv\Scripts\python.exe .\src\teams_caption_notes.py --allow-positional-fallback
```
